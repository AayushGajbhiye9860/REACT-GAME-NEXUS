...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill }) {
  ...
  const offsetValue = useTransform(progress, (p) => {
    return (((p + itemOffset) % 100) + 100) % 100;
  });

  const offsetDistance = useTransform(offsetValue, (v) => `${v}%`);

  // 3D perspective mapping: 
  // 0% (Left) -> 25% (Bottom/Front) -> 50% (Right) -> 75% (Top/Back) -> 100% (Left)
  const scale = useTransform(offsetValue, [0, 25, 50, 75, 100], [0.8, 1.2, 0.8, 0.5, 0.8]);
  const opacity = useTransform(offsetValue, [0, 25, 50, 75, 100], [0.7, 1, 0.7, 0.3, 0.7]);
  const zIndex = useTransform(offsetValue, [0, 24.9, 25.1, 74.9, 75.1, 100], [10, 20, 20, 0, 0, 10]);
  
  // Apply translateZ for true depth passing - increased range for clarity
  const translateZ = useTransform(offsetValue, [0, 25, 50, 75, 100], [0, 200, 0, -200, 0]);
    ...
    >
      ...
      >
        ...
        >
          ...
          {centerContent && (
            <div className="orbit-center-content" style={{ transform: 'translateZ(10px)' }}>
              {centerContent}
            </div>
          )}
        </div>
